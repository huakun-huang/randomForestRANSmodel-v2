...
{
    return up/std::max(fabs(up)+fabs(down), Foam::VSMALL);
}
...
void PINNkOmegaSSTBase<BasicEddyViscosityModel>::correct()
{
    ...
    {
        Info<<"Get the output features using PINN"<<endl;
        #include "/home/hhk/OpenFOAM/PINNv/PINNkOmegaSST/createFields.H"
        #include "/home/hhk/OpenFOAM/PINNv/PINNkOmegaSST/createFeatures.H"
        #include "/home/hhk/OpenFOAM/PINNv/PINNkOmegaSST/getInputFeatures.H"
        #include "/home/hhk/OpenFOAM/PINNv/PINNkOmegaSST/outputFeatures.H"
    
        Info<<"Run randomforest algorithm"<<endl;
        system("python /home/hhk/OpenFOAM/PINNv/include/regressionSolverRANS.py");
        data = fopen("outputResult.txt", "r");
        if(data!=NULL)
        {
            forAll(k_, celli)
            {
                double buffer=0;
                fscanf(data, "%lf", &buffer);
                Pkf_[celli] = buffer;
        ...
        {
            Info<<"cannot find the outputResult.txt"<<endl;
            //exit();
        }
    }
