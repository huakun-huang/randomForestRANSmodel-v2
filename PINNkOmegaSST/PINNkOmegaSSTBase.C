...
tmp<volScalarField> PINNkOmegaSSTBase<BasicEddyViscosityModel>::F1
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> PINNkOmegaSSTBase<BasicEddyViscosityModel>::F2() const
{
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> PINNkOmegaSSTBase<BasicEddyViscosityModel>::F3() const
{
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> PINNkOmegaSSTBase<BasicEddyViscosityModel>::F23() const
{
...
template<class BasicEddyViscosityModel>
void PINNkOmegaSSTBase<BasicEddyViscosityModel>::correctNut
(
...
template<class BasicEddyViscosityModel>
Foam::tmp<Foam::volScalarField> PINNkOmegaSSTBase<BasicEddyViscosityModel>::S2
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField::Internal> PINNkOmegaSSTBase<BasicEddyViscosityModel>::Pk
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField::Internal> PINNkOmegaSSTBase<BasicEddyViscosityModel>::epsilonByk
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField::Internal> PINNkOmegaSSTBase<BasicEddyViscosityModel>::GbyNu0
(
    const volTensorField& gradU,
...
template<class BasicEddyViscosityModel>
tmp<volScalarField::Internal> PINNkOmegaSSTBase<BasicEddyViscosityModel>::GbyNu
(
...
template<class BasicEddyViscosityModel>
tmp<fvScalarMatrix> PINNkOmegaSSTBase<BasicEddyViscosityModel>::kSource() const
{
...
template<class BasicEddyViscosityModel>
tmp<fvScalarMatrix> PINNkOmegaSSTBase<BasicEddyViscosityModel>::omegaSource() const
{
    return tmp<fvScalarMatrix>::New
    (
        omega_,
...
template<class BasicEddyViscosityModel>
tmp<fvScalarMatrix> PINNkOmegaSSTBase<BasicEddyViscosityModel>::Qsas
(
    const volScalarField::Internal& S2,
...
template<class BasicEddyViscosityModel>
PINNkOmegaSSTBase<BasicEddyViscosityModel>::PINNkOmegaSSTBase
(
    const word& type,
...
:
    ...
    (
        IOobject
        (
            IOobject::groupName("Pk", alphaRhoPhi.group()),
            this->runTime_.timeName(),
            this->mesh_,
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
    ...
    (
        IOobject
        (
            IOobject::groupName("Dk", alphaRhoPhi.group()),
            this->runTime_.timeName(),
            this->mesh_,
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
    ...
    (
        IOobject
        (
            IOobject::groupName("Pw", alphaRhoPhi.group()),
            this->runTime_.timeName(),
            this->mesh_,
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
    ...
    (
        IOobject
        (
            IOobject::groupName("Dw", alphaRhoPhi.group()),
            this->runTime_.timeName(),
            this->mesh_,
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
...
template<class BasicEddyViscosityModel>
void PINNkOmegaSSTBase<BasicEddyViscosityModel>::setDecayControl
(
...
template<class BasicEddyViscosityModel>
bool PINNkOmegaSSTBase<BasicEddyViscosityModel>::read()
{
    if (BasicEddyViscosityModel::read())
    {
		...
		critical_.readIfPresent(this->coeffDict());
...
template<class BasicEddyViscosityModel>
void PINNkOmegaSSTBase<BasicEddyViscosityModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
	...
	forAll(Pkf_, celli)
	{
		if(Pkf_[celli]>critical_.value())
			Pkf_[celli] = critical_.value();
		if(Pwf_[celli]>critical_.value())
			Pwf_[celli] = critical_.value();
		if(Dkf_[celli]>critical_.value())
			Dkf_[celli] = critical_.value();
		if(Dwf_[celli]>critical_.value())
			Dwf_[celli] = critical_.value();
	}

    {
		...
		Pw_.ref() = gamma*GbyNu0*Pwf_;
        Dw_.ref() = beta*omega_()*omega_*Dwf_;
        Pk_.ref() = Pk(G)*Pkf_;
        Dk_.ref() =  epsilonByk(F1, tgradU())*k_*Dkf_;
        ...
        (
         ...
         ==
            alpha()*rho()*gamma*GbyNu0*Pwf_
          - fvm::SuSp((2.0/3.0)*alpha()*rho()*gamma*divU, omega_)
          - fvm::Sp(alpha()*rho()*beta*omega_()*Dwf_, omega_)
          - fvm::SuSp
            (
...

    {
        ...
        (
         ...
         ==
          ...
          - fvm::Sp(alpha()*rho()*epsilonByk(F1, tgradU())*Dkf_, k_)
